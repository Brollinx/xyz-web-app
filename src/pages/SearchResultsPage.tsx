...
import { calculateDistance, cn } from "@/lib/utils"; // Import calculateDistance and cn
...
const SearchResultsPage = () => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          setCurrentCenter(userLoc); // Center map on user location initially
  ...
  useEffect(() => {
    const fetchProductResults = async () => {
      try {
        ...
        const fetchedResults: ProductWithStoreInfo[] = [];
        data.forEach((product: any) => {
          if (product.stores && product.stores.is_active && product.stores.latitude !== null && product.stores.longitude !== null) {
        ...
        if (userLocation) {
          fetchedResults.sort((a, b) => (a.distance || Infinity) - (b.distance || Infinity));
        }
    ...
    fetchProductResults();
  }, [searchQuery, userLocation]); // Re-fetch when searchQuery or userLocation changes
  ...
  // Memoize unique stores for map markers to avoid duplicate markers for the same store
  const uniqueStoresForMarkers = useMemo(() => {
    const seenStoreIds = new Set<string>();
    const uniqueMarkers: { id: string; lat: number; lng: number; name: string; address: string }[] = [];
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-gray-50 p-4">
      <div className="w-full max-w-4xl text-center space-y-6 mb-8">
        ...
        <div className="flex w-full items-center space-x-2 mx-auto">
          <Input
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter') {
                // Optionally trigger a search explicitly if needed,
                // but current setup re-fetches on searchQuery state change.
              }
            }}
          />
      ...
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            ...
            {uniqueStoresForMarkers.map((store) => (
              <Marker
                ...
                onClick={() => {
                  // Find the first product result associated with this store to set as selected
                  const firstProductInStore = productResults.find(pr => pr.storeId === store.id);
                  if (firstProductInStore) {
                    handleMarkerClick(firstProductInStore);
                  }
                }}
              />
            ))}
        ...
        {/* Scrollable List of Products/Stores */}
        <div className="md:col-span-1">
